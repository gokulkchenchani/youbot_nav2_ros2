#include "multi_goal_nav.h"

using namespace std::chrono_literals;

const std::string bt_xml_dir =
    ament_index_cpp::get_package_share_directory("behavior_tree") + "/config";

MultiGoalNav::MultiGoalNav(const std::string &nodeName) : Node(nodeName)
{
...
void MultiGoalNav::setup()
{
  ...
  timer_ = this->create_wall_timer(
      timer_period,
      std::bind(&MultiGoalNav::update_behavior_tree, this));

  rclcpp::spin(shared_from_this());
  rclcpp::shutdown();
}

void MultiGoalNav::create_behavior_tree()
{
...
void MultiGoalNav::update_behavior_tree()
{
...
int main(int argc, char **argv)
{
  ...
  auto node = std::make_shared<MultiGoalNav>("multi_goal_nav_node");
  node->setup();
